...
def gen_energy_and_gradient( img,grad,theta ,layer_colors, weights, img_spatial_static_target = None, scratches = None ):
    '''
...
#        numpy.savetxt("test1.csv", test, delimiter=",")
    ...
    def e( Y ):
        ...
        e += 10* E_flow( Y, L_flow, scratches )
    ...
    def g( Y ):
        ...
        grad_E_spatial_dynamic(Y, L_flow, gradient_term, scratches)
        gradient_term *= 10
...
def optimize( arr,grad,theta, colors,Y0, weights, img_spatial_static_target = None, scratches = None, saver = None ):
    '''
    ...
    e, g = gen_energy_and_gradient( arr,grad,theta,colors,weights, img_spatial_static_target = img_spatial_static_target, scratches = scratches )
...
def run_one( imgpath, orderpath, colorpath, outprefix, weightspath = None, save_every = None, solve_smaller_factor = None, too_small = None ):
    ...
    if solve_smaller_factor != 1:
        ...
        def optimize_smaller( solve_smaller_factor, large_arr,large_grad,large_theta , large_Y0, large_img_spatial_static_target ):
            ...
            ##small_grad = downsample(large_grad)
            small_grad = large_grad[::solve_smaller_factor, ::solve_smaller_factor]
            ##small_theta = downsample(large_theta)
            ...
            small_Y1 = optimize_smaller( solve_smaller_factor, small_arr,small_grad,small_theta, small_Y0, small_img_spatial_static_target )
            ...
            small_Y = optimize( small_arr, small_grad,small_theta,colors,small_Y1, weights, img_spatial_static_target = small_img_spatial_static_target, saver = saver )
        ...
        Y0 = optimize_smaller( solve_smaller_factor, arr,grad,theta, Y0, static )
    
    reset_saver( arr.shape )
    Y = optimize( arr,grad,theta,colors, Y0, weights, img_spatial_static_target = static, saver = saver )
