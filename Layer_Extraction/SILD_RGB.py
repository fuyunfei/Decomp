...
def E_flow( Y, L_flow, scratches = {} ):
    ## I don't see how to specify the output memory
    return dot( Y, L_flow.dot( Y ) )



def grad_E_flow( Y, L_flow, out, scratches = {} ):
    ## I don't see how to specify the output memory
    out[:] = L_flow.dot( Y )
    out *= 2

def E_edge( Y, L_edge, scratches = {} ):
    ## I don't see how to specify the output memory
    return dot( Y, L_edge.dot( Y ) )

def grad_E_edge( Y, L_edge, out, scratches = {} ):
    ## I don't see how to specify the output memory
    out[:] = L_edge.dot( Y )
    out *= 2
...
def gen_energy_and_gradient( img, layer_colors, weights, img_spatial_static_target = None,
                             scratches = None,grad = None,theta = None,edge=None):
    '''
    ...
    # pprint( weights )
    assert set( weights.keys() ).issubset( set([ 'w_polynomial', 'w_opaque', 'w_spatial_static', 'w_spatial_dynamic','w_flow','w_edge'  ]) )
    ...
    if 'w_spatial_dynamic' in weights:
        ...
        # import numpy
        # test = LTL.toarray()
        # numpy.savetxt("test1.csv", test, delimiter=",")
    ...
    if grad is not None:
        ...
        print '    Generating flow_L...'
        L_flow=flow_laplacian.gen_flow_laplacian(grad,theta,img.shape[0], img.shape[1]  )
        print '    Replicating L_flow.T*L_flow for all layers...'
        L_flow = L_flow.tocoo()
        shape = L_flow.shape
        ks = arange(num_layers)
        rows = (repeat(asarray(L_flow.row).reshape(L_flow.nnz, 1) * num_layers, num_layers, 1) + ks).ravel()
        cols = (repeat(asarray(L_flow.col).reshape(L_flow.nnz, 1) * num_layers, num_layers, 1) + ks).ravel()
        vals = (repeat(asarray(L_flow.data).reshape(L_flow.nnz, 1), num_layers, 1)).ravel()

        L_flow = scipy.sparse.coo_matrix((vals, (rows, cols)),
        ...
        print '...Finished.'
    if edge is not None:
        import edge_laplacian
        import scipy.sparse
        print '    Generating  L_edge...'
        L_edge=edge_laplacian.gen_edge_laplacian(img.shape[0], img.shape[1],edge)
        print '    Replicating L_edge.T*L_edge for all layers...'
        L_edge = L_edge.tocoo()
        shape = L_edge.shape
        ks = arange(num_layers)
        rows = (repeat(asarray(L_edge.row).reshape(L_edge.nnz, 1) * num_layers, num_layers, 1) + ks).ravel()
        cols = (repeat(asarray(L_edge.col).reshape(L_edge.nnz, 1) * num_layers, num_layers, 1) + ks).ravel()
        vals = (repeat(asarray(L_edge.data).reshape(L_edge.nnz, 1), num_layers, 1)).ravel()

        L_edge = scipy.sparse.coo_matrix((vals, (rows, cols)),
                              shape=(shape[0] * num_layers, shape[1] * num_layers)).tocsr()
    ...
    def e( Y ):
        e = 0.
        if grad is not None:
            e += weights['w_flow']* E_flow( Y, L_flow, scratches )

        if edge is not None:
            e += weights['w_edge'] * E_edge(Y, L_edge,scratches)
    ...
    def g( Y ):
        ...
        if grad is not None:
            grad_E_flow(Y, L_flow, gradient_term, scratches)
            gradient_term *= weights['w_flow']
            total_gradient += gradient_term

        if edge is not None:
            grad_E_edge(Y, L_edge, gradient_term, scratches)
            gradient_term *= weights['w_edge']
            total_gradient += gradient_term
...
def optimize( arr,colors,Y0, weights, img_spatial_static_target = None, scratches = None, saver = None, grad=None,theta=None,edge=None):
    '''
    ...
    e, g = gen_energy_and_gradient( arr,colors,weights, img_spatial_static_target = img_spatial_static_target, scratches = scratches,grad=grad,theta=theta,edge=edge)
...
def run_one( imgpath,edge_path, orderpath, colorpath, outprefix, weightspath = None, save_every = None, solve_smaller_factor = None, too_small = None ):
    '''
    ...
    import json, os,numpy,PIL.Image
    from PIL import Image
    import time
    from numpy import genfromtxt

    grad = genfromtxt('../g.csv', delimiter=',')

    theta = genfromtxt('../theta.csv', delimiter=',')
    edge= asfarray( Image.open(edge_path ))
    '''
     input g and  theta
    '''
    ...
    if weightspath is not None:
        ...
        weights = { 'w_polynomial': 375, 'w_opaque': 1., 'w_spatial_dynamic': 100.,'w_flow': 4. }
    ...
    if 'w_flow' in weights:
        weights['w_flow'] /= num_layers

    if 'w_edge' in weights:
        weights['w_edge'] /= num_layers
    
    if solve_smaller_factor != 1:
        assert solve_smaller_factor > 1
        
        def optimize_smaller( solve_smaller_factor, large_arr,large_grad,large_theta , large_Y0, large_img_spatial_static_target ):
            ...
            if large_grad is not None:
                small_grad = large_grad[::solve_smaller_factor, ::solve_smaller_factor]
                small_theta = large_theta[::solve_smaller_factor, ::solve_smaller_factor]
            ...
            small_Y1 = optimize_smaller( solve_smaller_factor, small_arr,None,None, small_Y0, small_img_spatial_static_target )
            ...
            small_Y = optimize( small_arr,colors,small_Y1, weights, img_spatial_static_target = small_img_spatial_static_target, saver = saver , grad=None, theta=None)
        ...
        #Y0 = optimize_smaller( solve_smaller_factor, arr,None,None, Y0, static )
    
    reset_saver( arr.shape )
    #Y = optimize( arr,colors, Y0, weights, img_spatial_static_target = static, saver = saver )
    Y = optimize(arr, colors, Y0, weights, img_spatial_static_target=static, saver=saver, grad=None, theta=None,edge=edge)
...
if __name__ == '__main__':
    ...
    image_path, edge_path, orderpath, color_path, output_prefix = args
    import time
    start=time.clock()
    run_one( image_path,edge_path, orderpath, color_path, output_prefix, weightspath = weightspath, save_every = save_every, solve_smaller_factor = solve_smaller_factor, too_small = too_small )
    end=time.clock()
